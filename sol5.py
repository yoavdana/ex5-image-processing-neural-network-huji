...
def train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples):
    ...
    epochs=num_epochs,validation_data=valid_data_set,validation_steps=num_valid_samples/batch_size,use_multiprocessing=True)
...
def restore_image(corrupted_image, base_model):
    ...
    return restored_image.reshape(height,width).astype('float64')
...
Answer1 = "the core of the implamentation is in the corupttion function,the rest of it is generic for all the models.the SR coruption pick a random integer between 2-4 resize the image by 1/the number ,then resize it back by the integer,when the risizsing is done we loose information from the image (sample out some pixels)'and as we do the inverese process this lost information can't be restored accuratly,and the missed pixels are beeing evaluated by the zoom function(i guess by statistical or intepoliation algorithm),and as a result we get a lower resolution image,finaly i used map coordinates to return to the original input shape.the out come of loosing information an cousing lower resolution image will a ssist the network to determine its weights for this specific task.  " #@param {type:"string"}
...
Answer2 = "for initial try i started with the given parameter from the blur model (batch_size=100,steps_per_epoch=100,num_epochs = 10  ,num_valid_samples = 1000,patch_size=16,num_chunnels=32),and number of res blocks =7.in this atempt max loss was 0.0054 and min loss was 0.0035,the validation loss was pretty close to those values,after the first epoch there was a major decrease in the loss from 0.0054 to 0.0037,and after that all epoches where about the same.that means the model is'nt improving ,to change it i tried to add more parameters to the model by taking patch_size=30,num_chunnel=60,in this atempt the loss decreased to 0.0033 and it looks like there is an improvment in each epoch,before adding more epochs i tiried to change patch_size and num_chunnel many times and i found the 30 and 60 giving the best results,i also tried to reduce or increase the res blocks number,and 7 giving me the best results.i added 5 more epochs and 100 more steps per epoch,the max loss is 0.0054 the min loss is 0.0032,the loss is stable and does not increasing between epoch but stop deacrising also,the val loss is very close to the train loss'the final parameters are : batch_size=100,steps_per_epoch=200,num_epochs = 15  ,num_valid_samples = 1000,patch_size=30,num_chunnels=60,and number of res blocks =7 " #@param {type:"string"}
...
Answer3 = "in both tasks (deblur an denoise),we can see that in general if we add more blocks the MSE loss is improving .in the case of the debluring model,1 res block is intend to underfit the data and it min loss is little les than 0.014,with 2 res blocks we can see a major improvment and it seams that the model will keep improving for more epochs,an intresting thing accure with 4 and 5 res blocks.it looks like 4 res blocks as a lower loss than 5,i think it's may point on the existance of some optimal number of res blocks and maybe 5 res blocks passes it.in addition it looks like if we will do more epochs the 4 and 5 will decreas more,the min aproximate values for the last cases are 0.008.in the case of the denoise model,it look like there ar better performance when adding more res blocks,but the model is not stable in case of 2,3,4 res blocks,the mse in those cases oscilates but decreas in gereral.in the 5 res blocks we can see much more stable MSE  but not as the debluring model.in addition it's look like there is a local minima aproximali in the 8th epoch for the 5 res case,but i assume that it will get back to the min value after more epochs.  " #@param {type:"string"}
