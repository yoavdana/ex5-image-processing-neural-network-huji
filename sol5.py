...
Original file is located at
    https://colab.research.google.com/drive/1mpacyMzA2l2h1swSq4-Gkw2cRFjFj0b5
...
"""###Version 1.1 - Last Update 07.01.2021
**Changelog:** 
...
def load_dataset(filenames, batch_size, corruption_func, crop_size):
    ...
    images_dict={}
    crop_row=crop_size[0]
    crop_col=crop_size[1]
    while True:
        target_batch=[]
        source_batch=[]
        for i in range(batch_size):
            rand_idx = random.randint(LEN_FILE)
            if filenames[rand_idx] not in images_dict:
                im=read_image(filenames[rand_idx],1)
                corrupt=corruption_func(im)
                images_dict[filenames[rand_idx]]=[im,corrupt]
            curr_im=images_dict[filenames[rand_idx]][0]
            curr_corrupt=images_dict[filenames[rand_idx]][1]
            rand_row,rand_col=random.randint(curr_im.shape[0]-crop_row),random.randint(curr_im.shape[1]-crop_col)
            sliced_im=curr_im[rand_row:rand_row+crop_row,rand_col:rand_col+crop_col]-0.5
            sliced_corrupt=curr_corrupt[rand_row:rand_row+crop_row,rand_col:rand_col+crop_col]-0.5
            target_batch.append(sliced_im)
            source_batch.append(sliced_corrupt)
        yield (np.array(target_batch),np.array(source_batch))
